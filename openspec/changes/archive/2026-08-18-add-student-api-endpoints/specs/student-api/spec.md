...
系统 SHALL 提供 `GET /api/warning/student/{student_id}` 端点，返回与该学生相关的预警通知列表，按创建时间倒序。仅限 `student` 角色访问自己的数据。

#### Scenario: 正常查询预警列表

- **WHEN** 学生请求 `GET /api/warning/student/{student_id}` 且 `student_id` 与当前登录学生一致
- **THEN** 系统返回该学生的预警记录列表，每项含 `warning_id`、`warning_type`、`level`、`title`、`content`、`status`、`created_at`
