"""ChemAI 评测体系 — pytest 共享 fixtures

复用 tests/conftest.py 的 fixtures，为评测场景提供数据库和客户端。
"""
...
def school(db_session: Session) -> School:
    s = School(name="评测第一中学", region="湖南省", address="长沙市岳麓区",
...
def teacher(db_session: Session, school: School) -> Teacher:
    t = Teacher(name="评测老师", phone="13800001111", email="eval@test.edu",
...
def teacher_account(db_session: Session, teacher: Teacher) -> Account:
    a = Account(username="eval_teacher", password_hash=hash_password("123456"),
...
def student(db_session: Session, class_: Class) -> Student:
    s = Student(name="评测学生", phone="13900002222", status="approved",
...
def student_account(db_session: Session, student: Student) -> Account:
    a = Account(username="eval_student", password_hash=hash_password("123456"),
...
def parent(db_session: Session) -> Parent:
    p = Parent(name="评测家长", phone="13700003333", email="eval_parent@test.com")
    db_session.add(p)
    db_session.commit()
    return p
...
def parent_account(db_session: Session, parent: Parent) -> Account:
    a = Account(username="eval_parent", password_hash=hash_password("123456"),
